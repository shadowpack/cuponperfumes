...
class RPDF extends PDF_HTML
{
    ...
    {
        $db = new db_core();
        $user = $db->reg_one("SELECT * FROM users INNER JOIN transacciones ON transacciones.id_user=users.id_user WHERE transacciones.id_transaccion = '".$this->id."'");
        $this->consulta = $db->reg_one("SELECT * FROM productos as p 
            ...
            WHERE transacciones.id_user='".$user['id_user']."' AND transacciones.statusPay='1' AND transacciones.id_transaccion='".$this->id."';");
        // Arial bold 15
    ...
    function bajada(){
        ...
        $this->SetXY(10, 71);
        $this->SetFont('Arial','',10);
        $this->WriteHTML($this->consulta['descripcion']);
        $this->SetDrawColor(210,211,205);
    ...
    function body(){
        ...
        $this->SetXY(10, 140);
        $this->SetFont('Arial','',10);
        $this->WriteHTML($this->consulta['condiciones']);
